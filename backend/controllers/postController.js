  ...
  async function getPostById(req, res) {
    try {
      ...
      // Only derive if summary doesn't exist in DB
      const derivedSummary = row.summary ? row.summary : (row.content ? String(row.content).slice(0, 160) : '');

      return res.json({ post: { ...row, summary: derivedSummary } });
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        db.query(
          ...
          WHERE id = ?`,
          [finalTitle, finalSlug, finalSummary, finalContent, finalFeaturedImage, finalFeatured, finalCategoryId, finalStatus, id],
          (err, r) => {
            if (err) return reject(err);
            resolve(r);
          }
